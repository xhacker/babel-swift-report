...
Evaluating a code conversion tool is difficult, because there are no obvious metrics for them. Typical metrics like performance are not important for code conversion tools. Instead, the most important qualities for code conversion tools are \emph{the support for input source code}, \emph{the correctness of conversion}, and \emph{the quality of converted code}. This chapter defines these terms attempts to evaluate these qualities in the following sections.

\section{Methodology}

To collect real world Objective-C code for evaluation, we chose most voted questions under \emph{objective-c} tag on Stack Overflow. If the question or answer contains code, we can use that for our evaluation. We've collected X pieces of code in total.

Because Babel Swift only supports converting code snippets, we did the two steps to process the code:
...
\begin{enumerate}
  ...
  \item If the code contains a function, we use the function body as the input for Babel Swift; we use the whole function for other conversion tools.
\end{enumerate}

We then run Babel Swift, objc2swift, Swiftify, and iSwift on the X inputs.

\section{The Support of Input Source Code}

If a conversion tool succeeded in converting a piece of source code, we say that it \emph{support} that piece of code. Table \ref{table:numcodesupported} shows number of code pieces each conversion tool supported.
...
\section{The Correctness of Conversion}

If the converted code and the original code have the same functionality, we say that the conversion is \emph{correct}. It is hard to write a general purpose tool to test the code automatically. So we evaluated the correctness of conversion by hand. Table \ref{table:correctness} shows the correctness of conversions for each code piece.
...
Finally, we convert all the test cases by hand. If the machine-converted code is \emph{identical} to the hand-converted, we say that the conversion is \emph{optimal}. By \emph{identical}, we do tolerate extra or lack of spaces. Table \ref{table:quality} shows the quality of converted code for each conversion tool.
